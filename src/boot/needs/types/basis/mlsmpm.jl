...
@inline function mls_moments(mpts::Point{T1,T2,D,CM}, mesh::Mesh{T1,T2,D}, basis::Basis{T1,T2,D,NN,K}, ip::T1) where {T1,T2,D,NN,K<:MLSBasis,CM}
    p2n = basis.p2n[ip]
    h   = mesh.prprt.h
    xp  = mpts.x[ip]
    D1  = D + 1
...
@inline function eval_basis(mpts::Point{T1,T2,D,CM}, mesh::Mesh{T1,T2,D}, basis::Basis{T1,T2,D,NN,K}, ip::T1, nn::T1) where {T1,T2,D,NN,K<:MLSBasis,CM}
    no = basis.p2n[ip][nn]
    if iszero(no)
...
@inline function eval_basis_all(mpts::Point{T1,T2,D,CM}, mesh::Mesh{T1,T2,D}, basis::Basis{T1,T2,D,NN,K}, ip::T1) where {T1,T2,D,NN,K<:MLSBasis,CM}
    D1 = D + 1
    w, P, Minv = mls_moments(mpts, mesh, basis, ip)
...
@kernel inbounds = true function shpfun!(mpts::Point{T1,T2,D,CM}, mesh::Mesh{T1,T2,D}, basis::Basis{T1,T2,D,NN,K}) where {T1,T2,D,NN,K<:MLSBasis,CM}
    p = @index(Global)
    if p ≤ mpts.nmp
