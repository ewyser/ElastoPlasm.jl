@views function MCRetMap!(mp,ϵIIp,cmParam,fwrkDeform)
    ...
    for p ∈ 1:mp.nmp
        ϕ,H,ϵII0 = mp.ϕ[p],cos(mp.ϕ[p])*cmParam.Hp,ϵIIp[p]
        c0,cr    = mp.c₀[p]+cmParam.Hp*ϵII0,mp.cᵣ[p]
        if c0<cr c0 = cr end
        ...
        if f>0.0
            ...
            mp.ϵ[:,:,p].= mutate(cmParam.Del\σ[:,p],0.5,:tensor)
            mp.ϵpII[p]  = ϵII 
            if fwrkDeform == :finite
