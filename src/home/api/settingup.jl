...
"""
    set_paths(newDir::String, path::String) -> Dict{Symbol, String}
...
# Description
The `set_paths` function is designed to help users organize their project directories by creating a structured set of subdirectories within a specified base path. The function first ensures that the specified main directory (`newDir`) exists within the provided base path (`path`). If `newDir` does not exist, it is created.
