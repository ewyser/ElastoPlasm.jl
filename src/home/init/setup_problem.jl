...
"""
    setup_problem(geom, solver, ic_points; te=0.0, tg=0.0, tep=0.0, thermal=false) -> MechanicalProblem
...
- `thermal::Bool=false`: (Optional) Build real thermal-phase state (`mesh.t`/`mpts.t`)
  instead of leaving both `nothing` — forwarded to `setup_mesh`/`setup_mpts`. Only
  `thermal_problem` sets this.
...
```
"""
function setup_problem(geom::Geometry{T1,T2,D}, solver::S, ic_points::Function; te=0.0,tg=0.0,tep=0.0,thermal::Bool=false) where {T1,T2,D,S<:AbstractSolver{T1,T2,D}}
    mesh = setup_mesh(geom, solver; thermal=thermal)
    mat  = setup_material_constants(solver)
    mpts = setup_mpts(mesh, solver, mat; geom = ic_points(mesh, mat, solver), thermal=thermal)
