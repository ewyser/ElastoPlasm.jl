@kernel inbounds = true function ΔJn(mpts::Point{T1,T2,D},mesh::Mesh{T1,T2,D},basis::Basis{T1,D}) where {T1,T2,D}
    p = @index(Global)
    if p ≤ mpts.nmp
        # buffering
...
@kernel inbounds = true function ΔJp(mpts::Point{T1,T2,D},mesh::Mesh{T1,T2,D},basis::Basis{T1,D},dim::T2) where {T1,T2,D}
    p = @index(Global)
    if p ≤ mpts.nmp
