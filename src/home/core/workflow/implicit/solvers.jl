...
- `nothing`
"""
function elastodynamic!(mpts::Point{T1,T2,D,E,R},mesh::Mesh{T1,T2,D},cmpr::NamedTuple,time::Time{T1,T2},instr::Instruction{T1,T2,D,S}) where {T1,T2,D,E,R,S<:ImplicitSolver}
    @warn "Elastodynamic workflow is not implemented for ImplicitSolver. Please use ExplicitSolver instead."
    return nothing
end  
"""
...
- `nothing`
"""
function elastoplastic!(mpts::Point{T1,T2,D,E,R},mesh::Mesh{T1,T2,D},cmpr::NamedTuple,time::Time{T1,T2},instr::Instruction{T1,T2,D,S}) where {T1,T2,D,E,R,S<:ImplicitSolver}
    @warn "Elastoplastic workflow is not implemented for ImplicitSolver. Please use ExplicitSolver instead."
    return nothing
end  
