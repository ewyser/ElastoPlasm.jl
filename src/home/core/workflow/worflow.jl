...
function elastoimplicit!(mpts::Point{T1,T2,D,E,R},mesh::Mesh{T1,T2,D},cmpr::NamedTuple,time::Time{T1,T2},instr::Instruction{T1,T2,D}) where {T1,T2,D,E,R}
    it,checks = T1(0), T2.(sort(collect(time.t[1]:instr.plot.freq:time.te)))
    prog = Progress(length(checks);dt=0.5,desc="Solving elastoimplicit...",barlen=10)
    for T ∈ checks
        while time.t[1] < T
            tic      = time_ns()
            g,dt     = get_spacetime(mpts,mesh,cmpr,time,T)
            # implicit MPM cycle: topology → PT implicit solve → permanent stress/F update
            ignite(mpts,mesh,instr)
            mapsto_implicit(mpts,mesh,cmpr,g,dt,instr)
            elasto(mpts,mesh,cmpr,dt,instr)
            time.t[1],it,toc = time.t[1]+dt,it+T1(1),(time_ns()-tic)
        end
        savlot(mpts,mesh,time.t[1],instr)
