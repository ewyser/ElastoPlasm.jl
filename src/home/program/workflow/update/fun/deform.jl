@views @kernel inbounds = true function finite_deform(mpts::Point{T1,T2},mesh::MeshSolidPhase{T1,T2},dt::T2) where {T1,T2}
    p = @index(Global)
    if p ≤ mpts.nmp 
        ...
        for i ∈ 1:mesh.prprt.dim , j ∈ 1:mesh.prprt.dim
            mpts.s.ΔFᵢⱼ[i,j,p] = mpts.δᵢⱼ[i,j]+(dt*mpts.s.∇vᵢⱼ[i,j,p])
        end
        for i ∈ 1:mesh.prprt.dim, j ∈ 1:mesh.prprt.dim
            for k ∈ 1:mesh.prprt.dim
                mpts.s.Fᵢⱼ[i,j,p] += mpts.s.ΔFᵢⱼ[i,k,p]*mpts.s.Fᵢⱼ[k,j,p]
            end
        end
        #=# compute incremental deformation and update
...
@views @kernel inbounds = true function infinitesimal_deform(mpts::Point{T1,T2},mesh::MeshSolidPhase{T1,T2},dt::T2) where {T1,T2}
    p = @index(Global)
    if p ≤ mpts.nmp 
        # compute velocity & displacement gradients
        mpts.s.∇vᵢⱼ[:,:,p].= T2(0.0)
