...
@views @kernel inbounds = true function infinitesimal_elast(mp,Del)
    ...
    if p ≤ mp.nmp 
        # calculate elastic strains & spins
        mp.s.ϵᵢⱼ[:,:,p] .= 0.5.*(mp.s.ΔFᵢⱼ[:,:,p]+mp.s.ΔFᵢⱼ[:,:,p]').-mp.s.δᵢⱼ[:,:] 
        mp.s.ωᵢⱼ[:,:,p] .= 0.5.*(mp.s.ΔFᵢⱼ[:,:,p]-mp.s.ΔFᵢⱼ[:,:,p]')
        # update cauchy stress tensor
        mp.s.σJᵢⱼ[:,:,p].= mutate(mp.s.σᵢ[:,p],1.0,:tensor)
        mp.s.σJᵢⱼ[:,:,p].= mp.s.σJᵢⱼ[:,:,p]*mp.s.ωᵢⱼ[:,:,p]'+mp.s.σJᵢⱼ[:,:,p]'*mp.s.ωᵢⱼ[:,:,p]
        mp.s.σᵢ[:,p]   .+= Del*mutate(mp.s.ϵᵢⱼ[:,:,p],2.0,:voigt).+mutate(mp.s.σJᵢⱼ[:,:,p],1.0,:voigt)
    end  
end
