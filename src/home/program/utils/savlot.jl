"""
    savlot(mpts::Point{T1,T2}, mesh::Mesh{T1,T2}, t::T2, instr::Dict)
...
- `instr::Dict`: Simulation instruction dictionary, must contain plotting options in `:plot`.
...
```
"""
@views function savlot(mpts::Point{T1,T2},mesh::Mesh{T1,T2},t::T2,instr::Dict) where {T1,T2}
